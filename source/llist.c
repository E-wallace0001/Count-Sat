...
link_node* sort_sub(link_node* l_list, int exception){
	int count = 0;
	link_node* temp;
	link_node * list=l_list->first;
	hash_t* bucket ;
	bool ret =0;
	link_node* saved;
	link_node* pre;
	...
	while(1){
		...
		bucket = hasht_create(257);
		
		for( int i = f_clause_size[list->data]; i != 0;i--){
			if(abs( f_variable_connections[ list->data ][ i ] ) == abs(exception) ){
				//	printf( " %i %i \n", f_variable_connections[list->data][i], exception);
				continue;
			}
			//add to hash table
			table_add( abs( f_variable_connections[ list->data ][ i ] ), bucket );
		...
		while(1){
			...
			count = 0; 
			ret 	= 0;
			//if the clause is equal or small, skip
			
			for ( int i = f_clause_size[ temp->data ]; i!=0; i--){
				// check if this is in the hash table
				if(  check_table( abs( f_variable_connections[ temp->data ] [i] ), bucket ) == 1 ) {
					count++;
				}
			}
			...
			if( count > 0 ){
				...
				// move the link, to just before this one
	...
	while(1){
		...
		CollectVariables(Group, GroupSet, &list,Coms);
			...
			while(1){
				...
				//look through clause group->Data
				for( int v = f_clause_size[Group->data]; v!=0; v--){
					if( ExistInSet( v, appended ) == 1 )
						continue;
						
					sze = ListSize(GroupSet[v]);
					
					if( sze <= count && count!=0 || count ==0){
						count = sze;
						var=v;
				...
				//if there is no clause, break from while
				if(count==0){
					break;
				}
				...
				// sort copy var is the position in the clause
				...
				//copy=sort_sub(copy, abs(f_variable_connections[Group->data][var]) );
...
link_node* AddLists(link_node* OpA, link_node* OpB,com_line* Coms){
	...
	while(OpA!=NULL){
		OpB=link_append(OpA->data,OpB, Coms);
		tmp = OpA;
		OpA=OpA->next;
		release_mem( tmp, Coms->link_pool);
	}
...
link_node* sort_match(link_node* l_list){

	int count = 0;
	link_node* temp;
	link_node * list=l_list->first->end;
	hash_t* bucket ;
	bool ret =0;
	link_node* saved;
	link_node* pre;
	...
	while(1){
		...
		while(1){
			...
			ret 	= 0;
			//if the clause is equal or small, skip
			if(f_clause_size[ temp->data ] <= f_clause_size[ list->data ] ){
				if( temp->next == NULL)
					break;
				temp = temp->next;
				continue;
			...
			if( count == f_clause_size[ list->data ] ){
				matched=1;
				saved = list->previous;
